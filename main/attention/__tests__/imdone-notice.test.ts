 ...
 *
 * The identity convention (#258) applies: an agent is shown as provider + NAME.
...
describe('planImDoneNotice', () => {
    it('names the WORKSPACE — the fact the toast was missing', () => {
        const n = planImDoneNotice({
            workspace: 'tynn.ai',
            agent: { provider: 'claude', name: 'caddy-instdir' },
            terminal: 'claude · caddy-instdir',
        });
        expect(`${n.title} ${n.body}`).toContain('tynn.ai');
    });

    it('names the AGENT by provider and name, so one of several is identifiable', () => {
        const n = planImDoneNotice({
            workspace: 'tynn.ai',
            agent: { provider: 'claude', name: 'caddy-instdir' },
            terminal: 'claude · caddy-instdir',
    ...
    it('distinguishes two agents that share a NAME across providers', () => {
        const claude = planImDoneNotice({ workspace: 'tynn.ai', agent: { provider: 'claude', name: 'tynn' } });
        const codex = planImDoneNotice({ workspace: 'tynn.ai', agent: { provider: 'codex', name: 'tynn' } });
    ...
    it('names the TERMINAL when its label is not already the agent', () => {
        const n = planImDoneNotice({
            workspace: 'tynn.ai',
            agent: { provider: 'claude', name: 'reviewer' },
            terminal: 'Terminal 3',
        });
    ...
    it('still says who finished when there is no workspace', () => {
        const n = planImDoneNotice({ agent: { provider: 'codex', name: 'scratch' } });
    ...
    it('says nothing about a host for a LOCAL finish', () => {
        const n = planImDoneNotice({ workspace: 'tynn.ai', agent: { provider: 'claude', name: 'x' } });
