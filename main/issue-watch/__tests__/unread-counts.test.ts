...
describe('countByKind', () => {
    ...
    it('counts ALL items by kind, ignoring seen_at', () => {
        const items = [
        ...
        expect(countByKind(items)).toEqual({ issue: 2, pr: 1, dependabot: 1 });
    });

    it('is zero for an empty list', () => {
        expect(countByKind([])).toEqual({ issue: 0, pr: 0, dependabot: 0 });
    });
});
