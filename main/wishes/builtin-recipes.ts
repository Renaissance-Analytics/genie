/**
 * First-party Wish bodies — the actions an UNATTENDED Wish is allowed to take.
 *
 ...
 * thing a system-triggered Wish can ever run is one of the recipes in this file:
 * in-repo TypeScript, reviewed when it was written, reachable only by id.
 *
 * That is the same trust boundary `main/flows/nodes.ts` draws for flow graphs —
...
import type { WishRecipe, WishRunContext } from './types';
...
/** Where files go when the Wish does not say. */
 ...
 */
const SELF_IGNORING = [
    '# Written by a Genie Wish. Everything in this folder is deliberately',
    '# untracked -- it holds files moved out of the repository to keep it light.',
    '*',
    '',
].join('\n');

function requireString(ctx: WishRunContext, key: string): string {
 ...
 * keeps the Wish working there; letting EXDEV surface would make it fail for
 ...
 */
async function relocateFile(ctx: WishRunContext): Promise<void> {
    ...
    if (!isInside(root, destinationDir)) {
        throw new Error(
            `"${relocateTo}" resolves outside the workspace. A Wish moves files WITHIN a ` +
    ...
    if (!fs.existsSync(source)) {
        ctx.set('relocated', false);
        ctx.set('relocatedReason', 'the file was gone by the time the Wish ran.');
        return;
    }
    ...
    // is exactly the window in which the Wish retriggers itself.
 ...
 * workspace, not for files it moved itself — is the Wish's triggers and filter,
 * not this recipe's business.
 */
export const relocateFileRecipe: WishRecipe = {
...
/** Every first-party Wish body, by id. */
export const BUILT_IN_WISH_RECIPES: ReadonlyMap<string, WishRecipe> = new Map([
