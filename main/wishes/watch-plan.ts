/**
 * PURE. Which workspaces need a filesystem watcher, given what the Wishes say.
 *
 ...
 * chance somebody might one day write a file Wish. The watch set is derived from
 * the declarations, the way `main/flows/schedule-plan.ts` derives which cron
 ...
 * Wish's triggers that producer covers. So a second file-shaped producer added
 ...
 * fire, so watching for it would be pure cost. A workspace-scoped Wish pointing
 * at a workspace that no longer exists is ignored for the same reason.
 */
export function planWishFileWatches(
    ...
    for (const wish of wishes) {
        ...
        const watchBacked = wish.triggers.some(
            (t) =>
                t.kind === 'event' &&
        ...
        if (wish.scope.kind === 'workspace') {
            wanted.add(wish.scope.workspaceId);
        } else {
            // A workstation- or app-scoped Wish reacts to files anywhere, which
