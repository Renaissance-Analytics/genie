/**
 * A system-triggered Wish runs its Recipe with NO HUMAN PRESENT.
 *
...
import { decideWishAdmission, UNATTENDED_SAFE_STEP_TYPES } from '../admission';
import type { WishRecipe, WishRecipeStep } from '../types';

const task: WishRecipeStep = {
    type: 'task',
    id: 'move',
    title: 'Move it',
    run: async () => {},
};
const terminal: WishRecipeStep = {
    type: 'terminal',
    id: 'sh',
    title: 'Run something',
    command: 'rm',
...
const form: WishRecipeStep = {
    type: 'form',
    id: 'ask',
    title: 'Which one?',
    fields: [{ key: 'name', label: 'Name' }],
};
const choice: WishRecipeStep = {
    type: 'choice',
    id: 'pick',
    title: 'Pick',
    options: [{ value: 'a', label: 'A' }],
};
const browser: WishRecipeStep = {
    type: 'browser',
    id: 'open',
    title: 'Open',
...
describe('an unattended run', () => {
    ...
    it('refuses steps that need a person to answer them, rather than hanging on one', () => {
        for (const step of [form, choice]) {
            const decision = decideWishAdmission(recipe(step), 'unattended');
    ...
    it('refuses a browser step — nobody is at the machine it would open on', () => {
        const decision = decideWishAdmission(recipe(browser), 'unattended');
        expect(decision.ok).toBe(false);
    ...
    it('refuses the WHOLE recipe when only one step is unsafe, before anything runs', () => {
        ...
        // half its work. `main/flows/runner.ts` states the same rule for graphs.
        const decision = decideWishAdmission(recipe(task, terminal, task), 'unattended');
        expect(decision.ok).toBe(false);
        expect(decision.refusals.map((r) => r.stepId)).toEqual(['sh']);
    });

    it('names every unsafe step, not just the first', () => {
        const decision = decideWishAdmission(recipe(terminal, form, task), 'unattended');
...
describe('an attended run', () => {
    it('admits every step type — a person is watching, and the existing approval gates apply', () => {
        ...
        const decision = decideWishAdmission(
...
describe('the unattended-safe set', () => {
    ...
    it('refuses a step type it has never heard of', () => {
        ...
        const strange = { type: 'teleport', id: 'x', title: 'Teleport' } as unknown as
            WishRecipe['steps'][number];
        const decision = decideWishAdmission(recipe(strange), 'unattended');
        expect(decision.ok).toBe(false);
