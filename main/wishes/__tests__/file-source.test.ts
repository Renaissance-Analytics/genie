...
import { FILE_ADDED_EVENT, buildFileAddedEvent, startWishFileSource } from '../file-source';
...
describe('startWishFileSource', () => {
    type Over = Partial<Parameters<typeof startWishFileSource>[0]>;

    function harness(over: Over = {}) {
        let deliver: ((e: FileWatchEvent) => void) | null = null;
        const emitted: WishEvent[] = [];
        const stop = startWishFileSource({
            subscribe: (l) => {
                deliver = l;
                return () => {
                    deliver = null;
    ...
    it('waits for a file still being written, so the size it reports is the real one', async () => {
        ...
        // appears — at zero bytes. A Wish filtering on "over 5 MB" would never
    ...
    it('collapses the duplicate events one file creation produces', async () => {
        ...
        // being written produces `change`s in between. Without this a Wish would
        // run several times for one file — and for a Wish that MOVES the file,
