 ...
 *
 * The HONESTY: a Wish that silently does not fire is the hardest failure in an
...
function registry() {
    const r = createWishEventRegistry();
    r.register({
        id: 'demo:happened',
        label: 'Demo',
...
function harness(opts: {
    wishes?: Wish[];
    recipe?: WishRecipe | null;
    guard?: WishLoopGuard;
}) {
    const logs: WishRunLog[] = [];
    const runtime = new WishRuntime({
        registry: registry(),
        guard: opts.guard ?? new WishLoopGuard(),
        listWishes: () => opts.wishes ?? [wish()],
...
const ranRecipe = (sink: string[]): WishRecipe => ({
    id: 'r',
    title: 'R',
    steps: [
        {
...
describe('a run has the event props and the Wish args in its context', () => {
    it('seeds args first and lets the event props win', async () => {
        const seen: string[] = [];
        const h = harness({
            wishes: [wish({ recipe: { kind: 'builtin', recipeId: 'r', args: { note: 'default' } } })],
...
describe('refusals stop everything, and say why', () => {
    ...
    it('records a blocked run when the loop guard holds it, without resolving a body', async () => {
        ...
        await runtime.emit({
            event: 'demo:happened',
            props: {},
            source: { kind: 'wish', wishId: 'w', runId: 'r1', depth: 0 },
        });
...
describe('runManually', () => {
    ...
    it('refuses a Wish with no manual trigger, a disabled one, and one that does not exist', async () => {
        const h = harness({
            wishes: [
                wish({ id: 'no-manual', triggers: [{ kind: 'event', event: 'demo:happened' }] }),
                wish({ id: 'off', enabled: false }),
            ],
            recipe: ranRecipe([]),
        });
...
describe('a Wish that triggers another Wish', () => {
    /** `a` announces something; `b` reacts to the announcement. */
    function chain(maxDepth: number) {
        const reached: number[] = [];
        const guard = new WishLoopGuard({ maxDepth });
        const announce: WishRecipe = {
            id: 'announce',
            title: 'Announce',
            steps: [
                {
            ...
            steps: [
                {
                    ...
                    run: async (ctx) => {
                        ...
                        reached.push(source?.kind === 'wish' ? source.depth : 0);
                    },
                },
            ],
        };
        ...
        const wishes = [
            wish({ id: 'a', recipe: { kind: 'builtin', recipeId: 'announce' } }),
            wish({ id: 'b', recipe: { kind: 'builtin', recipeId: 'record' } }),
        ];
        const runtime = new WishRuntime({
            registry: registry(),
            guard,
    ...
    it('does not let a Wish trigger itself through its own emission', async () => {
        const emitted: string[] = [];
        const guard = new WishLoopGuard();
        const selfAnnouncing: WishRecipe = {
            id: 'r',
            title: 'R',
            steps: [
                {
...
describe('two Wishes on one event', () => {
    it('both run, and each is logged', async () => {
        const sink: string[] = [];
        const h = harness({
            wishes: [wish({ id: 'a' }), wish({ id: 'b' })],
        ...
        expect(logs.map((l) => l.wishId).sort()).toEqual(['a', 'b']);
