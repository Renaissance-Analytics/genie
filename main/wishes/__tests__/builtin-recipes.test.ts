...
import { runWishTasks } from '../recipe';
import type { WishDeclaredEffect, WishRunContext } from '../types';
...
function context(values: Record<string, unknown>) {
    ...
    const declared: WishDeclaredEffect[] = [];
...
const run = (ctx: WishRunContext) => runWishTasks(relocateFileRecipe, ctx);
...
describe('relocating a file', () => {
    it('uses the default folder when the Wish does not name one', async () => {
    ...
    it('declares the destination BEFORE it writes anything', async () => {
        ...
        // and declaring is exactly the window in which the Wish retriggers
