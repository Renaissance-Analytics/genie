 ...
 *
 * `FILE_ADDED_EVENT` declares the props a Wish may filter on, and the code below
 * is what actually emits them. Splitting those across files is how a registry
 * entry ends up promising a prop nothing sends — a Wish that looks armed and can
 ...
 * one new file runs the Wish three times, and for a Wish that MOVES the file the
 ...
 * copied in is 0 bytes when Genie first hears about it, so a Wish filtering on
...
import type { WishEvent, WishEventDefinition } from './types';

export const FILE_ADDED_EVENT: WishEventDefinition = {
    id: 'files:added',
 ...
 */
export function buildFileAddedEvent(input: FileAddedInput): WishEvent | null {
...
export interface WishFileSourceDeps {
    ...
    emit: (event: WishEvent) => void | Promise<void>;
...
export function startWishFileSource(deps: WishFileSourceDeps): () => void {
    ...
    async function observe(raw: FileWatchEvent, workspaceId: string, key: string): Promise<void> {
        ...
        try {
            ...
            // The emitter runs Wishes and has no caller to return to. Reported
            // rather than left to become an unhandled rejection.
            console.error('[wishes] file source emit failed', e);
        }
    }
