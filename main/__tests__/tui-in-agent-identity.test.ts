 ...
 *
 * The owner's rule: *"provider should be tui because the tui is what determines
 * the provider and supports TUIs that can use any provider, so provider itself
 * is not important for agent identity. Unique should be on workspace, tui,
 * name."*
 *
 * v55 went the other way: it collapsed `(workspace, provider, name)` to
 ...
 * satisfy the new key — zero duplicate `(workspace, provider, name)` groups and
...
function addAgent(
    ...
    d.prepare(
        `INSERT INTO workspace_agents
           (id, workspace_id, provider, name, purpose, role, reachability, wake_on_dm,
...
describe('agent identity is (workspace, tui, name) — v60', () => {
    ...
    it('still allows only ONE workspace agent per workspace', () => {
        ...
        d.prepare(
            `INSERT INTO workspace_agents
               (id, workspace_id, provider, name, purpose, role, reachability, wake_on_dm, created_at, updated_at)
