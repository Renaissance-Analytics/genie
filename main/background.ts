 ...
 * a person noticing an absence is the slowest, least reliable monitor there is.
 * It is also how genie#551 was found rather than reported.
 *
 ...
 * after an upgrade Genie may have no window on screen at all, which is the case
 * a renderer-side surface cannot cover.
 *
 * `planRestoreNotice` decides what is worth saying — failures only; a skip is a
 * decision the restore made correctly.
 */
function reportRestoreFailures(outcomes: readonly DrainRestoreOutcome[]): void {
    try {
        ...
        // console line above already carries the same facts.
            ...
            await devLifecycle()?.onBoot({
                ...
                beforeResume: async () => {
                    ...
                    try {
                        ...
                        // person noticed its absence — which is exactly how this
                        // bug was found. A SKIP stays in the log; it is a
                        // decision the restore made correctly.
