...
/**
 * A grant, as the AUTHORITY a `gapp`-scoped flow runs under.
 *
 * `decideFlowAdmission` takes an authority now, not a grant, because a flow can
 * also run as the USER — see `authority.test.ts`. This file is entirely about the
 * app case, so the helper wraps it once rather than at every call.
 */
const asApp = (g: AppGrant | null): FlowAuthority => ({ kind: 'app', grant: g });
...
describe('a graph the app was granted', () => {
    ...
    it('admits a graph with no Genie nodes at all', () => {
        const d = decideFlowAdmission(graph(node('a', '@particle-academy/branch')), asApp(grant()));
...
describe('a graph reaching past the grant', () => {
    ...
    it('refuses a node naming an ungrantable tool, however the app was granted', () => {
        const d = decideFlowAdmission(
            ...
            asApp(grant({ capabilities: ['hosting', 'files', 'notify', 'terminals'] })),
        );
    ...
    it('refuses a node in Genie’s namespace that names no real tool', () => {
        ...
        const d = decideFlowAdmission(graph(node('typo', 'genie.mangeTerminals')), asApp(grant()));
    ...
    it('still admits a Fancy builtin, which is not Genie’s namespace at all', () => {
        ...
        expect(decideFlowAdmission(graph(node('b', '@particle-academy/merge')), asApp(grant())).allowed).toBe(
            true,
        );
    });
    ...
    it('admits a node targeting a workspace the app WAS scoped to', () => {
        const d = decideFlowAdmission(
            ...
            asApp(grant({ scope: 'workspaces', workspaces: ['ws-app', 'ws-two'] })),
        );
...
describe('a graph with no standing to run at all', () => {
    ...
    it('refuses an empty graph rather than pretending it ran', () => {
        const d = decideFlowAdmission(graph(), asApp(grant()));
...
describe('malformed input, which is the normal case for a stored graph', () => {
    ...
    it('survives a graph that is not a graph', () => {
        for (const junk of [null, undefined, 'nope', 42, [], {}] as unknown[]) {
            expect(() => decideFlowAdmission(junk as FlowGraphLike, asApp(grant()))).not.toThrow();
            expect(decideFlowAdmission(junk as FlowGraphLike, asApp(grant())).allowed).toBe(false);
        }
    });
    ...
    it('survives nodes that are not nodes', () => {
        ...
        expect(() => decideFlowAdmission(junk, asApp(grant()))).not.toThrow();
    });
    ...
    it('ignores a node whose kind is not a string', () => {
        ...
        expect(decideFlowAdmission(junk, asApp(grant())).refusals).toEqual([]);
    });
    ...
    it('refuses a node whose workspace target is not a string', () => {
        ...
        expect(decideFlowAdmission(junk, asApp(grant())).allowed).toBe(false);
    });
});
