 ...
 * Flow moves the file it just moved, forever. So every assertion here has a
...
function event(props: Record<string, string | number>, source: FlowEvent['source']): FlowEvent {
    return { event: 'demo:happened', props, source };
}

describe('an echo of a Flow’s own action does not retrigger it', () => {
    it('re-attributes the echo to the run that caused it, and blocks only that Flow', () => {
        ...
        expect(echo.source).toEqual({ kind: 'flow', flowId: 'mover', runId: 'run-1', depth: 1 });

        expect(guard.admit('mover', echo).ok).toBe(false);

        // POSITIVE CONTROL: the guard is not a blanket mute. Another Flow that
    ...
    it('matches an echo on the declared props only, ignoring the rest', () => {
        ...
        expect(echo.source).toMatchObject({ kind: 'flow', flowId: 'mover' });
    ...
    it('lets a declared echo expire, so a stale entry cannot mute a later event', () => {
        let now = 1_000;
        const guard = new FlowLoopGuard({ echoWindowMs: 5_000, now: () => now });
        guard.declareEffect({
            flowId: 'mover',
            runId: 'run-1',
            event: 'demo:happened',
...
describe('a chain of Flows cannot run away', () => {
    it('stops at the depth limit and admits everything below it', () => {
        const guard = new FlowLoopGuard({ maxDepth: 3 });

        for (let depth = 0; depth < 3; depth++) {
            const e = event({}, { kind: 'flow', flowId: 'other', runId: 'r', depth });
            expect(guard.admit('mine', e).ok, `depth ${depth} should be admitted`).toBe(true);
        }

        const tooDeep = event({}, { kind: 'flow', flowId: 'other', runId: 'r', depth: 3 });
    ...
    it('deepens the chain by one for the events a run causes', () => {
        const guard = new FlowLoopGuard();
        const triggering = event({}, { kind: 'flow', flowId: 'a', runId: 'r1', depth: 1 });
    ...
    it('never lets a Flow be triggered by its own emission, at any depth', () => {
        const guard = new FlowLoopGuard();
        const own = event({}, { kind: 'flow', flowId: 'mine', runId: 'r', depth: 0 });
        ...
        const theirs = event({}, { kind: 'flow', flowId: 'theirs', runId: 'r', depth: 0 });
        expect(guard.admit('mine', theirs).ok).toBe(true);
    });
});

describe('the circuit breaker is a backstop, not the mechanism', () => {
    it('quarantines a Flow that runs too often, and forgets once the window passes', () => {
        let now = 0;
        const guard = new FlowLoopGuard({ maxRunsPerWindow: 3, runWindowMs: 1_000, now: () => now });
        ...
        // POSITIVE CONTROL 1: another Flow is unaffected — the breaker is per-Flow.
