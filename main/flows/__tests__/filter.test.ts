 ...
 * first is ordinary, the second is a Flow that will never fire and must be
 * caught when it is written.
 */

import { describe, expect, it } from 'vitest';
import { matchesFlowFilter, validateFlowFilter, FlowFilterError } from '../filter';
import { FILE_ADDED_EVENT } from '../file-source';
import type { FlowEventDefinition, FlowFilterOp } from '../types';
...
const CASES: Array<[FlowFilterOp, string, unknown, unknown]> = [
...
describe('every operator, in both directions', () => {
    it.each(CASES)('%s on %s', (op, prop, matching, notMatching) => {
        expect(
            matchesFlowFilter({ all: [{ prop, op, value: matching as never }] }, props),
            `${op} should match`,
        ).toBe(true);
        expect(
            matchesFlowFilter({ all: [{ prop, op, value: notMatching as never }] }, props),
            `${op} should not match`,
        ).toBe(false);
    });
});
...
describe('the groups compose', () => {
    it('matches everything when there is no filter — a trigger may have no predicate', () => {
        expect(matchesFlowFilter(undefined, props)).toBe(true);
        expect(matchesFlowFilter({}, props)).toBe(true);
    });

    it('requires all of `all`', () => {
        const clause = { prop: 'sizeBytes', op: 'gt' as const, value: 5_242_880 };
        expect(matchesFlowFilter({ all: [clause] }, props)).toBe(true);
        expect(
            matchesFlowFilter(
    ...
    it('requires one of `any`', () => {
        ...
        expect(
            matchesFlowFilter({ any: [{ prop: 'extension', op: 'eq', value: 'png' }] }, props),
        ).toBe(false);
    });
    ...
    it('does not match on a prop this event did not carry', () => {
        ...
        expect(matchesFlowFilter({ all: [{ prop: 'absent', op: 'eq', value: 1 }] }, props)).toBe(
            false,
        );
    });
});
...
describe('a broken predicate is a fault, not a non-match', () => {
    it('throws for an unknown operator rather than quietly never matching', () => {
        expect(() =>
            matchesFlowFilter(
                { all: [{ prop: 'sizeBytes', op: 'approximately' as FlowFilterOp, value: 1 }] },
                props,
            ),
        ).toThrow(FlowFilterError);
    });
    ...
    it('throws when the comparison value is the wrong shape for the operator', () => {
        ...
        expect(() =>
            matchesFlowFilter({ all: [{ prop: 'extension', op: 'in', value: 'mp4' }] }, props),
        ).toThrow(FlowFilterError);
    });
});

describe('validateFlowFilter, against what the event declares', () => {
    ...
    it('catches the misspelling that would otherwise be a silent night', () => {
        const errors = validateFlowFilter(
    ...
    it('catches an operator applied to the wrong type', () => {
        expect(
            validateFlowFilter({ all: [{ prop: 'relPath', op: 'gt', value: 1 }] }, DEF).join(' '),
        ).toContain('cannot be applied to string');
    });

    it('catches a value of the wrong type', () => {
        expect(
            validateFlowFilter({ all: [{ prop: 'sizeBytes', op: 'eq', value: 'big' }] }, DEF).join(
                ' ',
            ),
        ).toContain('number');
    });
    ...
    it('catches a list where a single value belongs, and the reverse', () => {
        expect(
            validateFlowFilter({ all: [{ prop: 'extension', op: 'eq', value: ['a'] }] }, DEF),
        ).toHaveLength(1);
        expect(
            validateFlowFilter({ all: [{ prop: 'extension', op: 'in', value: 'a' }] }, DEF),
        ).toHaveLength(1);
    });

    it('catches a regex that does not compile', () => {
        expect(
            validateFlowFilter({ all: [{ prop: 'relPath', op: 'matches', value: '([' }] }, DEF).join(
                ' ',
            ),
