 ...
 *
 * There is no IPC and no UI, so nothing in the app can currently CREATE a Flow —
 * the grouped menu (users and agents, grouped by purpose, sorted by trigger) and
 * GApp-authored Flows are both deferred. The model, the store and the runtime
 * are complete and tested, so the menu plugs into `store.ts` and
 * `match.isFlowVisibleTo` when it lands rather than needing either rewritten. A
...
export { FlowRuntime, type FlowRunLog } from './runtime';
export {
    listFlows,
    getFlow,
    upsertFlow,
