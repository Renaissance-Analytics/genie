 ...
 *
 * The schema says so — v55 collapsed `UNIQUE (workspace_id, tui, name)` to
 * `UNIQUE (workspace_id, name)` and moved the tui onto `agent_runtimes`,
 * because an agent switching drivers is the same agent. The code did not
 * follow: `savedAgentKey` still returned `{tui}:{name}`, so the DB and the
 * keying disagreed about what an agent IS.
 *
 * That disagreement was visible. `agentRef` is emitted as the canonical
 * machine-facing identity, composed from the FRONTED tui — so switching an
 ...
 *
 * The tui is ADDRESSING, not identity: which driver is in front right now.
...
describe('agentRef', () => {
    ...
    it('still carries the chat session, which IS addressing', () => {
        expect(agentRef({ tui: 'claude', name: 'tynn', chatSessionId: 'c1' })).toBe('tynn:c1');
    });
    ...
    it('degrades to the bare key before a chat id exists', () => {
        ...
        expect(agentRef({ tui: 'codex', name: 'tynn', chatSessionId: null })).toBe('tynn');
    });
});
...
describe('parseAgentRef', () => {
    ...
    it('still reads a LEGACY tui-prefixed ref', () => {
        // Agents were told the old shape. Reading one has to keep working, and
        // the tui it names is recovered rather than mistaken for the name.
        const parsed = parseAgentRef('claude:tynn:c1');
        expect(parsed).toMatchObject({ tui: 'claude', name: 'tynn', chatSessionId: 'c1' });
    });
    ...
    it('does not treat an agent NAMED like a tui as a legacy ref', () => {
