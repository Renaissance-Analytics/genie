...
export interface ManagedProcessInfo {
    ...
    autostart: boolean;
    /**
     * The user PAUSED this process (genie#407) — stopped it deliberately, and it
     * stays down through the next launch whatever `autostart` says. An agent that
     * finds a process `stopped` and starts it is UNDOING that, so it is reported
     * here rather than left for the agent to infer from a status that cannot
     * distinguish the two.
     */
    paused?: boolean;
...
export interface ManageProcessResult {
    ...
    pending?: boolean;
    /** logs: the tail of that process's output, control sequences stripped —
     *  the evidence of WHY it stopped (genie#655). */
    log?: string;
    /** What Genie actually established. start/restart: a spawn that survived a
     *  short window is "alive when Genie last looked", never a health check.
     *  stop: whether the exit was OBSERVED or there was nothing running to
     *  begin with. disable/delete: carries a stop that could not be confirmed,
     *  since the action itself still happened. Relay it instead of reporting
     *  the service as working (or as gone). */
    note?: string;
}
...
export interface ManageSiteResult {
    ...
    pending?: boolean;
    /**
     * Did THIS call re-ask whether the site is serving, or is `ready` the last
     * answer Genie happened to have?
     *
     * `status` re-probes before answering (`manager.refresh`) — that is the
     * genie#305 fix — and a settled `create`/`start`/`restart` probed as part of
     * starting. `list` deliberately does NOT: every other action's envelope
     * calls it too, so a probe there would put a network round-trip behind all
     * of them. Neither do `logs`, `stop`, or an `update` that needed no restart.
     *
     * Absent means `ready` is a REMEMBERED answer of unknown age, so nothing
     * built on this result may state the site is serving — see
     * {@link manageSiteSummary}.
     */
    probed?: boolean;
...
export interface DevServiceEngineInfo {
     ...
     *  one. Not the same question as `image`, which is what Genie pins today. */
    runningImage?: string;
    /** The container is running an image OTHER than the pinned one, because an
     *  engine is adopted by name and never by image — so a Postgres started
     *  before the pin changed keeps the old one until somebody recreates it.
     *  This is why an extension Genie advertises can still fail to install on a
     *  build that carries it. Absent (not `false`) when the runtime reported no
     *  image: "unknown" and "current" are different answers. Recreating is the
     *  WORKSTATION operator's action — it restarts the engine for every
     *  workspace holding it — so what an agent does with this is SAY it. */
    staleImage?: boolean;
}
...
export function manageSiteSummary(result: ManageSiteResult): string {
    ...
    if (target) {
        ...
        if (target.ready) {
            // "is serving" is a claim about NOW, and only a call that re-probed
            // has grounds for it (CONTRIBUTING.md, "Never report a success you
            // have not verified"). `list` and `logs` hand back whatever `ready`
            // was last set to — the start-time snapshot genie#305 removed from
            // `status`, still live one action over — so there the claim is
            // narrowed to what is known, and points at the call that re-asks.
            if (result.probed) return `${target.name} is serving at ${where}.${locally}`;
            return (
                `${target.name} was serving at ${where} when Genie last checked — this call did not re-probe it, ` +
                `so that answer may be any age. \`manageSite {action:'status', id:'${result.affectedId}'}\` re-asks.${locally}`
            );
        }
...
export async function handleMcpMessage(
    ...
    switch (msg.method) {
        ...
        case 'tools/list': {
            ...
            // `manageSite` and `manageService` both have Host-native paths. Keep
            // them advertised whenever their Host capability is wired; individual
            // container-backed actions report the runtime install hint at call time.
            return ok(msg.id, {
                tools: [
                    ...CORE_TOOLS.filter(
                        (t) => t !== MANAGE_SERVICE_TOOL || Boolean(ctx.manageService),
                    ),
                    ...pluginTools,
                ],
            });
