...
describe('the notice for a ForceTheQuestion answer', () => {
    const answer = inboxNoticeText({ from: 'You', urgency: 'urgent', kind: 'ftq-answer', mode: 'manual' });
    ...
    it('leaves an ordinary DM notice exactly as it was', () => {
        ...
        const dm = inboxNoticeText({ from: 'claude · tynn', urgency: 'normal', mode: 'manual' });
    ...
    it('leaves a channel notice exactly as it was', () => {
        const chan = inboxNoticeText({ from: 'someone', urgency: 'urgent', channel: 'build', mode: 'manual' });
