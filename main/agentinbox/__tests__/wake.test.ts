...
describe('wakeNudgeText', () => {
    it('is a benign, self-describing one-liner (singular/plural)', () => {
        expect(wakeNudgeText(1, 'manual')).toContain('1 unread AgentInbox message;');
        expect(wakeNudgeText(1, 'manual')).toContain('receive');
        expect(wakeNudgeText(3, 'manual')).toContain('3 unread AgentInbox messages;');
        // Never reports zero.
        expect(wakeNudgeText(0, 'manual')).toContain('1 unread');
    });
});
