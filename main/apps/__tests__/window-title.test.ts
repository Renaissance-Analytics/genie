...
describe('gappWindowTitle', () => {
    it('prefixes the app’s name and carries the page’s own title', () => {
        expect(gappWindowTitle('Trader', 'Positions', 'https://trader.gapp/')).toBe(
            '[Trader] - Positions',
        );
    });
    ...
    it('is just the app when the page has not said anything', () => {
        ...
        expect(gappWindowTitle('Trader', '', 'https://trader.gapp/')).toBe('[Trader]');
        expect(gappWindowTitle('Trader', '   ', 'https://trader.gapp/')).toBe('[Trader]');
    });
    ...
    it('refuses a title that is really the page’s URL', () => {
        ...
        expect(gappWindowTitle('Trader', 'trader.gapp', 'https://trader.gapp/')).toBe('[Trader]');
        expect(gappWindowTitle('Trader', 'trader.gapp/positions', 'https://trader.gapp/')).toBe(
            '[Trader]',
        );
        expect(gappWindowTitle('Trader', 'https://trader.gapp/positions', 'https://trader.gapp/')).toBe(
            '[Trader]',
        );
    });
    ...
    it('refuses a title carrying ANY origin, not only this app’s', () => {
        ...
        expect(gappWindowTitle('Trader', 'https://example.com/thing', 'https://trader.gapp/')).toBe(
            '[Trader]',
        );
        expect(gappWindowTitle('Trader', 'localhost:5173', 'https://trader.gapp/')).toBe('[Trader]');
    });
    ...
    it('keeps a real title that merely mentions a dot', () => {
        ...
        expect(gappWindowTitle('Trader', 'Trader v2.1', 'https://trader.gapp/')).toBe(
            '[Trader] - Trader v2.1',
        );
        expect(gappWindowTitle('Trader', 'P&L · today', 'https://trader.gapp/')).toBe(
            '[Trader] - P&L · today',
        );
    });
    ...
    it('keeps the prefix stable however odd the app’s name is', () => {
        ...
        expect(gappWindowTitle('My App [beta]', 'Home', 'https://x.gapp/')).toBe(
            '[My App [beta]] - Home',
        );
    });
