...
export function getFlowIn(d: Database.Database, id: string): FlowRow | null {
    const raw = d
        .prepare<[string], RawFlow | undefined>(`SELECT ${COLUMNS} FROM gapp_flows WHERE id = ?`)
        .get(id);
...
export function listFlowsForAppIn(d: Database.Database, appId: string): FlowRow[] {
    return d
        .prepare<[string], RawFlow>(`SELECT ${COLUMNS} FROM gapp_flows WHERE app_id = ? ORDER BY name`)
        .all(appId)
        .map(toFlow);
}
...
export function deleteFlowIn(d: Database.Database, id: string): void {
    d.prepare('DELETE FROM gapp_flows WHERE id = ?').run(id);
}
