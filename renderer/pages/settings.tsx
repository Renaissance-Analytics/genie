 ...
 */
/**
 * Tynn auto-provisioning — per-workspace panel inside Agent MCP.
 *
 * Surfaces where the active workspace stands: unlinked (pick a Tynn project),
 * signed-out (sign in on the Connections tab), or linked (agent token written
 * into .mcp.json, with a Re-provision/refresh). Provisioning itself is
 * "auto on open" — this panel is the manual control + status mirror.
 */
function TynnProvisionPanel({ activeWorkspace }: { activeWorkspace?: string }) {
    ...
    const refresh = async () => {
        ...
        const ws = (await api().workspaces.list()).find((w) => w.id === activeWorkspace);
    ...
    const linkAndProvision = async () => {
        ...
        try {
            ...
            const r = await api().tynn.provision(path, true);
            setMsg(provisionMessage(r));
            await refresh();
        } finally {
            setBusy(false);
        }
...
function AgentMcpSection({
    ...
    return (
        <Card style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <TynnProvisionPanel activeWorkspace={activeWorkspace} />
