    ...
    return (
        <Modal open onClose={onClose} size="xl">
            <div className="ws-settings">
                ...
                <LocalSitesPanel workspaceId={workspace.id} />
...
/**
 * Serve-local-sites (Phase B): the per-repo `.gen` tunnel allowlist. Discovers
 * this host's loopback dev sites (hosts-file parse + loopback probe) and lists
 * them per site with an enable toggle, an editable `.gen` name, and a scheme/port
 * override. Nothing is tunnelled until a site is enabled (infra names default
 * off). HOST-SOURCED: in a remote window the `sites.*` calls route to the host
 * (remote-bridge.ts), so the list + edits reflect the machine being driven.
 *
 * The global master switch (`local_sites_enabled`) lives in Settings → Work Mode;
 * this panel is the second, per-repo opt-in on top of it.
 */
function LocalSitesPanel({ workspaceId }: { workspaceId: string }) {
    ...
    return (
        <Section
            title="Local sites"
            sub="Serve this host's loopback dev sites (e.g. tynn.test) to a remote as *.gen"
            action={
                <Action
                    size="sm"
                    variant="ghost"
