...
/** Monitor + cog — the System Workspace toggle (machine-level, configurable). */
export const IconMonitorCog = ({ size = 16, ...p }: Props) =>
    wrap(
        size,
        <>
            <path d="M12 17v4" />
...
/** Magic wand — "Run a recipe" (launch a plugin/built-in recipe wizard). Mirrors
 *  the Fancy `wand-2` glyph used inside the WizardModal header. */
export const IconWand = ({ size = 16, ...p }: Props) =>
    wrap(
        size,
        <>
...
/** Envelope with a question mark — the pending-questions top-bar inbox. */
export const IconMailQuestion = ({ size = 16, ...p }: Props) =>
    wrap(
        size,
        <>
